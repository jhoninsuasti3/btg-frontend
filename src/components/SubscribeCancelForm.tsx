...
import { subscribeToFund, cancelFundSubscription } from '../services/fundsService';

interface SubscribeCancelFormProps {
  selectedFundId: string;
}

const SubscribeCancelForm: React.FC<SubscribeCancelFormProps> = ({ selectedFundId }) => {
  ...
  return (
    <div>
      <button onClick={handleSubscribe} disabled={!selectedFundId}>Suscribirse</button>
      <button onClick={handleCancel} disabled={!selectedFundId}>Cancelar Suscripción</button>
      {message && <p>{message}</p>}
    </div>
  );
};
